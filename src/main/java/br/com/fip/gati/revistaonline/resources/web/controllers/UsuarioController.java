...
public class UsuarioController {
	...
	public UsuarioController(Environment env, AdminManager roles,
			UsuarioService usuarioService,
			UsuarioRepositorio usuarioRepositorio, UsuarioLogado usuarioLogado,
	...
	public void salvar(Usuario usuario, Autor autor) {
		...
		if (usuario != null) {
			if (usuario.getSenha() == null
					|| usuario.getConfirmacaoSenha() == null) {
				Controllers.includeValidationError(this.valitador, "senha",
	...
	@Post
	public void atualizarSenha(Usuario usuario, String senhaAtual,
		...
		if (senhaAtual.equals("") || novaSenha.equals("")
			...
			this.valitador.add(new ValidationMessage(
					"Todos os campos devem ser preenchidos", "Error"));
		} else if (!ShaEncrypt.hash(senhaAtual,
				this.environment.get("encryption.salt")).equals(senhaAnterior)) {
			...
			this.valitador
					.add(new ValidationMessage(
							"A senha precisa ter numeros,letras minusculas e letras maiusculas",
							"Error"));
		}
	...
	public void delRoleAdmin(Usuario usuario) {
		...
		if (this.usuarioLogado.getUsuarioInfo().getID().equals(usuario.getId())) {
			result.include(
					"errors",
					Arrays.asList(new ValidationMessage(
							"Não é permitido alterar as próprias permissões",
							"erro"))).redirectTo(this).listar();
		} else {
			try {
				...
				result.include(
						"errors",
						Arrays.asList(new ValidationMessage(e.getMessage(),
								"erro"))).redirectTo(this).listar();
			}
		}
